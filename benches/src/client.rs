...
impl RestaurantApiClient {
    ...
    pub async fn send_requests(&self, num_requests: usize, concurrency: usize) {
        ...
        let requests = stream::iter(0..num_requests).map(|_| {
            ...
            tokio::spawn(async move {
                ...
                match response {
                    Ok(resp) => {
                        println!("Response: {:?} - Time taken: {:?}", resp.status(), request_duration);
                    },
                    Err(e) => {
        ...
        requests.buffer_unordered(concurrency).for_each(|_| async {}).await;

        let total_duration = start_time.elapsed();
        let avg_req_per_sec = num_requests as f64 / total_duration.as_secs_f64();
        println!("All requests completed in {:?}. Average requests per second: {:.2}", total_duration, avg_req_per_sec);
    }
    ...
    fn generate_random_restaurant(id: String) -> serde_json::Value {
        ...
        let names = ["Bodega", "Maggie", "Akbar Joojeh", "Pasta House", "Burger Town"];
        let tags = vec![vec!["Tacos", "Burritos"], vec!["Pizza", "Pasta"], vec!["Sushi", "Tempura"]];
        let images = [
